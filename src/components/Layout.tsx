  ...
  return <div className={`app-shell ${widokKompaktowy ? 'is-compact' : ''}`}>
    <aside className={`sidebar ${menuMobilneOtwarte ? 'is-open' : ''}`}>
      <div className="sidebar-scroll">
        ...
        <NavLink className="sidebar-add" to="/spolecznosc"><Icon name="plus" size={18}/><span>Dodaj pomysł</span></NavLink>
        ...
        <button className="sidebar-profile" type="button" aria-label="Otwórz profil użytkownika"><span className="profile-avatar">K</span><span className="profile-copy"><b>Gość</b><small>Zaloguj się do konta</small></span><Icon name="arrow" size={15}/></button>
    ...
    <main className="main-area">
      <header className="topbar">
        ...
        <label className="topbar-search"><Icon name="search" size={16}/><input placeholder="Szukaj projektów, pomysłów…" aria-label="Szukaj projektów i pomysłów"/></label>
        <div className="topbar-actions"><NavLink className="button primary compact topbar-add" to="/spolecznosc"><Icon name="plus" size={16}/><span>Dodaj projekt</span></NavLink><button className="topbar-theme" aria-label="Powiadomienia"><Icon name="bell" size={17}/></button><button className="topbar-theme" onClick={zmienMotyw} aria-label={`Zmień motyw. Obecnie: ${etykietaMotywu}`}><Icon name={motyw === 'dark' ? 'moon' : 'sun'} size={17}/></button><button className="account-button"><Icon name="user" size={16}/><span>Zaloguj się</span></button></div>
      </header>

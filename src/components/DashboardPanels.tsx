...
export function SekcjaRecenzji() {
  return <section className="section-block reviews-section"><div className="section-head"><div><span className="section-kicker">SPRAWDZONE PO KAPIEMU</span><h2>Recenzje, testy i porównania</h2><p>Wyróżnione materiały z konkretnym werdyktem.</p></div><Link className="text-link desktop-link" to="/recenzje">Zobacz wszystkie <Icon name="arrow" size={16}/></Link></div><div className="reviews-grid">{reviews.map((recenzja,indeks) => <article className={`review-card ${indeks === reviews.length - 1 ? 'locked-review' : ''}`} key={recenzja.title}><div className="review-mark">{recenzja.title.slice(0,2).toUpperCase()}</div><div className="review-copy"><span>{recenzja.category}</span><h3>{recenzja.title}</h3><p>{recenzja.verdict}</p><small>{recenzja.author} · {recenzja.date}</small></div><div className="score"><strong>{recenzja.score}</strong><small>/10</small></div></article>)}</div></section>;
}

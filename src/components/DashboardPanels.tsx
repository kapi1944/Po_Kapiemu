...
export function PanelPodsumowania() {
  ...
  return <section className="summary-layout" aria-label="Szybki przeglÄ…d i status ogĂłlny">
    <div className="summary-grid"><article className="summary-card technical"><Icon name="projects" size={19}/><div><b>{liczbaAktywnychProjektow}</b><span>Aktywne projekty</span></div><small>{liczbaProjektow} projektĂłw Ĺ‚Ä…cznie</small></article><article className="summary-card music"><Icon name="vote" size={19}/><div><b>{liczbaAktywnychGlosowan}</b><span>Aktywne gĹ‚osowania</span></div><small>{liczbaGlosow} oddanych gĹ‚osĂłw</small></article><article className="summary-card blocks"><Icon name="activity" size={19}/><div><b>{liczbaAktywnosci}</b><span>Nowe aktywnoĹ›ci</span></div><small>Od ostatniej wizyty</small></article><article className="summary-card experimental"><Icon name="content" size={19}/><div><b>{liczbaMaterialow}</b><span>Do publikacji</span></div><small>MateriaĹ‚y w kolejce</small></article></div>
    <article className="overall-status"><div className="status-ring" style={{background:`conic-gradient(${tloRingu})`}}><div className="status-ring-content"><strong>{liczbaProjektow}</strong><span>projektĂłw Ĺ‚Ä…cznie</span></div></div><div className="status-legend"><h2>Status ogĂłlny</h2><ul>{rozkladStatusow.map(grupa => <li key={grupa.etykieta}><i style={{background:grupa.kolor}}/><span>{grupa.etykieta}</span><b>{grupa.liczba}</b></li>)}</ul></div></article>
  </section>;
}

export function FeedAktywnosci() {
  return <section className="activity-panel" aria-labelledby="naglowek-aktywnosci"><div className="compact-heading"><div><span className="section-kicker">NA Ĺ»YWO</span><h2 id="naglowek-aktywnosci">Teraz siÄ™ dzieje</h2></div><Link className="text-link" to="/tresci">CaĹ‚y feed <Icon name="arrow" size={15}/></Link></div><div className="activity-list">{aktywnosci.map(({ikona,kolor,przed,wyroznienie,po,czas}) => <div key={czas}><span className={`activity-icon ${kolor}`}><Icon name={ikona} size={15}/></span><p>{przed}{wyroznienie && <b>{wyroznienie}</b>}{po}<small>{czas}</small></p></div>)}</div></section>;
}

export function KartaGlosowania() {
  const liczbaGlosow = poll.options.reduce((suma, opcja) => suma + opcja.votes, 0);
  return <section className="poll-card feature-card" aria-labelledby="naglowek-glosowania"><div className="card-topline"><span className="tag hot">AKTYWNE GĹOSOWANIE</span><span className="binding">NiewiÄ…ĹĽÄ…ce</span></div><h2 id="naglowek-glosowania">{poll.question}</h2><p>WybĂłr spoĹ‚ecznoĹ›ci pomaga ustaliÄ‡ kolejnoĹ›Ä‡ pracy.</p><div className="poll-options">{poll.options.map((opcja, indeks) => { const procent = Math.round(opcja.votes / liczbaGlosow * 100); return <KomunikatFunkcji key={opcja.label} klasaKontenera="komunikat-funkcji--pelna-szerokosc" klasaPrzycisku={`poll-result category-${opcja.category}`} etykieta={`Informacja o gĹ‚osowaniu: ${opcja.label}`} tytul="GĹ‚osowanie na dashboardzie" opis="Oddanie gĹ‚osu jest dostÄ™pne w module GĹ‚osowania. Ten widok pokazuje aktualne wyniki."><span className="poll-result-head"><span className="poll-letter">{String.fromCharCode(65 + indeks)}</span><span>{opcja.label}</span><b>{procent}%</b></span><span className="poll-result-track"><span style={{width:`${procent}%`}}/></span></KomunikatFunkcji>; })}</div><div className="poll-footer"><span>{liczbaGlosow} gĹ‚osĂłw</span><Link to="/glosowania">ZagĹ‚osuj <Icon name="arrow" size={15}/></Link></div></section>;
}

export function ModulyDashboardu({ czyZalogowany = false }: { czyZalogowany?: boolean }) {
  return <section className="dashboard-modules" aria-label="Dodatkowe moduĹ‚y dashboardu">
    <article className="collaboration-card"><div><span className="section-kicker">WSPĂ“ĹPRACA</span><h2>ZrĂłbmy coĹ› dobrego razem.</h2><p>Jasne zasady, praktyczne formaty i projekty pasujÄ…ce do Po Kapiemu.</p><div className="collaboration-chips"><span>SprzÄ™t do testĂłw</span><span>Sponsoring</span><span>Afiliacja</span><span>WspĂłlny projekt</span></div></div><Link className="button secondary compact" to="/wspolpraca">Napisz w sprawie wspĂłĹ‚pracy <Icon name="arrow" size={15}/></Link></article>
    <article className="categories-card"><span className="section-kicker">KATEGORIE</span><h2>Szybki skrĂłt</h2><div className="category-list"><Link className="technical" to="/kategorie#techniczne">Techniczne</Link><Link className="music" to="/kategorie#muzyczne">Muzyczne</Link><Link className="blocks" to="/kategorie#klockowe">Klockowe</Link><Link className="experimental" to="/kategorie#eksperymentalne">Eksperymentalne</Link></div></article>
    <article className={`reminders-card ${!czyZalogowany ? 'guest-lock-shell guest-lock-shell--module' : ''}`}>
      <div className={!czyZalogowany ? 'guest-lock-shell__content' : undefined} inert={!czyZalogowany || undefined} aria-hidden={!czyZalogowany || undefined}><div className="compact-heading"><div><span className="section-kicker">NADCHODZI</span><h2>Kolejna publikacja</h2></div></div><ul><li><i/><span>Kulisy budowy VidEdit Studio</span><small>PiÄ…tek Â· 18:00</small></li></ul></div>
      {!czyZalogowany && <BlokadaDlaGoscia kompaktowa tytul="Kolejna publikacja dla zalogowanych" opis="Zaloguj siÄ™, aby zobaczyÄ‡ termin i peĹ‚nÄ… listÄ™ nadchodzÄ…cych publikacji."/>}
    </article>
  </section>;
}

export function SekcjaPublikacji({ czyZalogowany = false }: { czyZalogowany?: boolean }) {
  return <section className="section-block"><div className="section-head"><div><span className="section-kicker">NOWE</span><h2>Ostatnio opublikowane</h2><p>Filmy, aktualizacje, artykuĹ‚y i materiaĹ‚y do pobrania.</p></div><Link className="text-link desktop-link" to="/tresci">Wszystkie treĹ›ci <Icon name="arrow" size={16}/></Link></div><div className="content-grid">{contentItems.map((material,indeks) => {
    ...
    return <div className="guest-lock-shell guest-lock-shell--content" key={material.title}>
      ...
      <BlokadaDlaGoscia kompaktowa tytul="WiÄ™cej treĹ›ci po zalogowaniu" opis="Zaloguj siÄ™, aby zobaczyÄ‡ dalsze publikacje i materiaĹ‚y do pobrania."/>
    </div>;
  })}</div></section>;
}
  ...
  return <section className="section-block reviews-section">
    <div className="section-head">
      <div>
        ...
        <h2>Recenzje, testy i porĂłwnania</h2>
        <p>WyrĂłĹĽnione materiaĹ‚y z konkretnym werdyktem.</p>
      </div>
        ...
        <div className="guest-lock-shell guest-lock-shell--review">
          ...
          >
            <article className="review-card">
              ...
              >
                <span>Recenzja</span>
                <h3>Ukryty materiaĹ‚</h3>
                <p>Dalsza zawartoĹ›Ä‡ dostÄ™pna po zalogowaniu.</p>
              </div>
          ...
          <BlokadaDlaGoscia
            kompaktowa
            tytul="WiÄ™cej testĂłw po zalogowaniu"
            opis="Zaloguj siÄ™, aby zobaczyÄ‡ kolejne recenzje, testy i porĂłwnania."
          />
        </div>
      }
    </div>

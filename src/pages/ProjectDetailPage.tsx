...
export function ProjectDetailPage() {
  ...
  return <div className="page-wrap inner-page">
    ...
    <div className="detail-grid"><section className="detail-card"><span className="section-kicker">NAJWAŻNIEJSZE</span><h2>Co już jest w projekcie?</h2><ul className="feature-list">{projekt.highlights.map(element => <li key={element}><span>✓</span>{element}</li>)}</ul></section><section className="detail-card"><span className="section-kicker">AKTUALIZACJE</span><h2>Ostatnie zmiany</h2><div className="timeline-item"><b>{projekt.lastUpdated}</b><p>Przygotowano publiczną prezentację projektu na stronie Po Kapiemu.</p></div><div className="timeline-item"><b>Poprzednio</b><p>Rozwój funkcji i porządkowanie podstawowego workflow.</p></div></section><section className="detail-card"><span className="section-kicker">GŁOSOWANIA</span><h2>Decyzje społeczności</h2><p>Tu pojawią się głosowania związane bezpośrednio z projektem.</p><button className="button secondary compact">Brak aktywnych głosowań</button></section><section className="detail-card"><span className="section-kicker">MATERIAŁY</span><h2>Pliki i publikacje</h2><p>Dokumentacja, materiały do pobrania oraz filmy powiązane z projektem.</p><Link className="text-link" to="/tresci">Przejdź do treści <Icon name="arrow" size={15}/></Link></section></div>
  </div>;
}
